...
bot_init = function(on_reload) -- The function run when the bot is started or reloaded.
	...
	print(colors('%{blue bright}Plugins Loaded Count :'), colors('%{magenta}'..#plugins))
	    print(colors('%{cyan}Admin bot : '), colors('%{magenta}'..'('..config.admin..')'))
...
on_chosen_inline_result_receive = function(inline)
 if not inline then
  api.sendMessage(config.admin, 'Shit, a loop without chosen inline!')
  return
 end
 ...
 --count the number of inlines
   ...
   if v.faction then
   if v.ftriggers then
    for k,w in pairs(v.ftriggers) do
     local blocks = match_pattern(w, inline.query)
     if blocks then
      print(colors('\nChosen Inline info:\t %{red bright}'..get_from(inline)..'%{reset}\n%{cyan bright}Date -> ('..os.date('On %A, %d %B %Y at %X')..')%{reset}'))
      if blocks[1] ~= '' then
       not_match = 0
            print(colors('%{green bright}Chosen Inline Match found:'), colors('%{blue bright}'..w))
			--client:incr('InlineNums')
           end
      local success, result = pcall(function()
       return v.faction(inline, blocks)
      end)
      if not success then
       print(inline.query, result)
       save_log('errors', result, inline.from.id or false, false, inline.query or false)
...
while is_started do -- Start a loop while the bot should be running.
	...
	if res then
		...
		elseif msg.chosen_inline_result then
			on_chosen_inline_result_receive(msg.inline_query)
		 end
		end
	end
end
